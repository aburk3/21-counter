...
const BlackjackTable = ({
  ...
  const statusKind = (status: string) =>
    status === "bust" || status === "loss"
      ? "bad"
      : status === "twenty_one" || status === "blackjack" || status === "win"
        ? "good"
        : "neutral";
    ...
    <TableWrap $skin={skin}>
      ...
      <Group>
        <HandRow>
          ...
          {activeRound?.resolved && HAND_STATUS_LABELS[activeRound.dealer_status] ? (
          ...
          <Seats>
            {(activeRound?.user_hands ?? []).map((hand, handIndex) => (
              <Seat
                ...
                $isPrimary
                $isActive={
                  activeRound?.phase === "user_turn" && activeRound.active_hand_index === handIndex
                }
              >
                <SeatHeader>
        ...
        <HandRow>
          ...
          <Seats>
            {(activeRound?.other_hands ?? []).map((seat, seatIndex) => (
              <Seat key={`seat-${seatIndex}`} $isActive={false}>
